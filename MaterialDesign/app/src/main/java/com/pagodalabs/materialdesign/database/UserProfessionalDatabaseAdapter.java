...
public class UserProfessionalDatabaseAdapter{
	DatabaseHelper databaseHelper;

	public UserProfessionalDatabaseAdapter(Context context) {
	...
	public long insertProfessional(Professional professional) {
		...
		insert = sqliteDb.insert(DatabaseHelper.TABLE_NAME_USER_PROFESSIONAL, null, values);
		}
		return insert;
	}
	...
	public int updateProfessional(Professional professional){
		SQLiteDatabase sqliteDb = databaseHelper.getWritableDatabase();
		ContentValues values = professionalValues(professional);
		return sqliteDb.update(DatabaseHelper.TABLE_NAME_USER_PROFESSIONAL, values, DatabaseHelper.COLUMN_PROFESSIONAL_ID+" = "+professional.getProfessionalId(), null);
	}
	...
	public Professional getDefaultProfessional(){
		...
		Cursor cursor = sqliteDb.query(DatabaseHelper.TABLE_NAME_USER_PROFESSIONAL, columns, DatabaseHelper.COLUMN_KEY_ID+" = " + 1, null, null, null, null);
	...
	public Professional getByProfessionalId(Integer professionalId){
		...
		String[] columns = {DatabaseHelper.COLUMN_PROFESSIONAL_ID, DatabaseHelper.COLUMN_CATEGORY_ID, DatabaseHelper.COLUMN_FIRST_NAME, DatabaseHelper.COLUMN_LAST_NAME, 
							DatabaseHelper.COLUMN_EMAIL, DatabaseHelper.COLUMN_PHONE, DatabaseHelper.COLUMN_DESCRIPTION, DatabaseHelper.COLUMN_ACTIVATION};
		Cursor cursor = sqliteDb.query(DatabaseHelper.TABLE_NAME_USER_PROFESSIONAL, columns, DatabaseHelper.COLUMN_PROFESSIONAL_ID+" = " + professionalId, null, null, null, null);
		while(cursor.moveToNext()){
			professional = initProfessional(cursor);
		}
		return professional;
	...
	public int delete(Integer id){
		SQLiteDatabase sqliteDb = databaseHelper.getWritableDatabase();
		return sqliteDb.delete(DatabaseHelper.TABLE_NAME_USER_PROFESSIONAL, DatabaseHelper.COLUMN_PROFESSIONAL_ID+" = "+id+"", null);
	}
	...
	public List<Professional> showAll(){
		...
		String[] columns = {DatabaseHelper.COLUMN_PROFESSIONAL_ID, DatabaseHelper.COLUMN_CATEGORY_ID, DatabaseHelper.COLUMN_FIRST_NAME, DatabaseHelper.COLUMN_LAST_NAME, 
							DatabaseHelper.COLUMN_EMAIL, DatabaseHelper.COLUMN_PHONE, DatabaseHelper.COLUMN_DESCRIPTION, DatabaseHelper.COLUMN_ACTIVATION};
		Cursor cursor = sqliteDb.query(DatabaseHelper.TABLE_NAME_USER_PROFESSIONAL, columns, null, null, null, null, null);
		while(cursor.moveToNext()){
